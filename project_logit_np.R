...
dat$assigned.labels[dat$assigned.labels == "Nociceptive"] <- "1"
dat$assigned.labels[dat$assigned.labels == "Peripheral Neuropathic"] <- "0"
dat$assigned.labels[dat$assigned.labels == "Central Neuropathic"] <- "0"
...
pdf("plot_var_imp_np.pdf",width=11,height=8)
varImpPlot(fit.rf)
dev.off()
...
fit<-glm(assigned.labels~X8+X12+X9+X11+X27+X3+X2+X13+X6+X24,data=dat,family="binomial")
#fit<-glm(assigned.labels~X8+X12+X9+X11+X27+X3,data=dat,family="binomial")
...
#fit<-glm(assigned.labels~X6+X2+X9+X8+X12+X27,data=dat,family="binomial")
...
pdf("roc_plot_np.pdf",width=11,height=8)
plot(res$FPR,res$Sens,xlab="False Positive Rate (FPR)",ylab="True Positive Rate (TPR)",type="s")
abline(a=0,b=1,lty=3)
