...
dat$assigned.labels[dat$assigned.labels == "Peripheral Neuropathic"] <- "1"
...
pdf("plot_var_imp_pn.pdf",width=11,height=8)
varImpPlot(fit.rf)
dev.off()
...
fit<-glm(assigned.labels~X12+X9+X3+X27+X29+X17+X8+X35+X15+X28,data=dat,family="binomial")

summary(fit)

#fit<-glm(assigned.labels~X27+X8+X15+X23,data=dat,family="binomial")
...
pdf("roc_plot_pn.pdf",width=11,height=8)
plot(res$FPR,res$Sens,xlab="False Positive Rate (FPR)",ylab="True Positive Rate (TPR)",type="s")
abline(a=0,b=1,lty=3)
